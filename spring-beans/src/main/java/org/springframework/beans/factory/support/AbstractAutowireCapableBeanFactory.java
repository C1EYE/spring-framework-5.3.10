...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// 解析获取 beanDefinition 对应 class 类型
		Class<?> resolvedClass = resolveBeanClass(mbd, beanName);
		if (resolvedClass != null && !mbd.hasBeanClass() && mbd.getBeanClassName() != null) {
			// 这里对 beanDefinition 做了一层深拷贝
			// 主要是在后面的 bean后置处理器可能会对 beanDefinition 修改
			mbdToUse = new RootBeanDefinition(mbd);
			mbdToUse.setBeanClass(resolvedClass);
		}

		try {
			// 检查是否有方法覆盖，如果有的话检查方法覆盖是否有重载
		...
		try {
			// 回调 InstantiationAwareBeanPostProcessor#postProcessBeforeInstantiation 方法
			// 如果上一步返回结果不为 null，继续回调
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// 如果允许暴露三级缓存实例
		if (earlySingletonExposure) {
			// 那么会去一级或二级缓存取出，这里不会去三级缓存获取，因为第二个参数为 false
			Object earlySingletonReference = getSingleton(beanName, false);
			// 如果 getSingleton 获取到了 bean 实例不为 null
			if (earlySingletonReference != null) {
				// 首先判断初始化前的对象实例和初始化后的对象实例是否一样
				if (exposedObject == bean) {
					// 如果一样，说明在初始化这个步骤没有后处理器修改过该对象
					// 那么就把从 getSingleton 获取到的对象作为最终对象返回
		...
		try {
			// 将 bean 实例注册到容器销毁钩子函数当中
			// 当容器销毁时会来调用 bean 的生命周期销毁方法
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
		...
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				...
				if (targetType != null) {
					// 回调 InstantiationAwareBeanPostProcessor 接口
					// postProcessBeforeInstantiation 方法
					bean = applyBeanPostProcessorsBeforeInstantiation(targetType, beanName);
					if (bean != null) {
						// 回调 BeanPostProcessor 接口 postProcessAfterInitialization 方法
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		// 判断是否有 InstantiationAwareBeanPostProcessor 实现类
		...
		if (hasInstAwareBpps) {
			...
			for (InstantiationAwareBeanPostProcessor bp : getBeanPostProcessorCache().instantiationAware) {
				// 处理 理@Autowired、@Resource、@Value 进行依赖注入
	...
	protected Object initializeBean(String beanName, Object bean, @Nullable RootBeanDefinition mbd) {
		...
		if (mbd == null || !mbd.isSynthetic()) {
			// BeanPostProcessor postProcessBeforeInitialization 回调
