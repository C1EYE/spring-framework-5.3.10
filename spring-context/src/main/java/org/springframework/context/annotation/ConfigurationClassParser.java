...
class ConfigurationClassParser {
	...
	public void parse(Set<BeanDefinitionHolder> configCandidates) {
		for (BeanDefinitionHolder holder : configCandidates) {
			...
			// 这里主要针对不同的 beanDefinition 做了一个处理
			// 然后统一都会封装成一个 ConfigurationClass 对象去解析
			try {
	...
	protected void processConfigurationClass(ConfigurationClass configClass, Predicate<String> filter) throws IOException {
		// 解析配置是否有 @Condition 注解，有的话调用match方法判断是不是需要跳过解析此配置类
		...
		// 一值往上寻找配置类的父类解析，直到为Null为止
		SourceClass sourceClass = asSourceClass(configClass, filter);
		do {
			// 解析 @Component @PropertySources @PropertySource @ComponentScans @ComponentScan @ImportResource 注解
	...
	protected final SourceClass doProcessConfigurationClass(
		...
		// 解析 @PropertySources 和 @PropertySource 注解，并将注解的 properties 属性文件添加到容器的环境变量对象中
		...
		// 解析 @ComponentScans 和 @ComponentScan 注解
		Set<AnnotationAttributes> componentScans = AnnotationConfigUtils.attributesForRepeatable(
				sourceClass.getMetadata(), ComponentScans.class, ComponentScan.class);
		if (!componentScans.isEmpty() &&
				!this.conditionEvaluator.shouldSkip(sourceClass.getMetadata(), ConfigurationPhase.REGISTER_BEAN)) {
			for (AnnotationAttributes componentScan : componentScans) {
				// 扫描 componentScan 所指定的路径，将扫描到的所有类封装成 BeanDefinitionHolder 返回
				Set<BeanDefinitionHolder> scannedBeanDefinitions =
						this.componentScanParser.parse(componentScan, sourceClass.getMetadata().getClassName());
				// 建立所有扫描到的 beanDefinition
				for (BeanDefinitionHolder holder : scannedBeanDefinitions) {
					BeanDefinition bdCand = holder.getBeanDefinition().getOriginatingBeanDefinition();
					if (bdCand == null) {
						bdCand = holder.getBeanDefinition();
					}
					// 判断扫描到的 beanDefinition 是不是一个配置类，如果是的话，继续递归继续该配置类
	...
	private void processMemberClasses(ConfigurationClass configClass, SourceClass sourceClass,
		...
		if (!memberClasses.isEmpty()) {
			...
			for (SourceClass memberClass : memberClasses) {
				// 判断当前遍历到的子类是否是一个配置类 并且这个子类的类名不能和外部类的类名一样
			...
			for (SourceClass candidate : candidates) {
				if (this.importStack.contains(configClass)) {
					...
					try {
						// 解析获取到的内部类，这是一个递归调用，直到配置类没有任何可解析内部类为止
		...
		if (checkForCircularImports && isChainedImportOnStack(configClass)) {
			...
			try {
				for (SourceClass candidate : importCandidates) {
					// 判断当前遍历到的这个被导入类是 ImportSelector 类型吗
					if (candidate.isAssignable(ImportSelector.class)) {
						Class<?> candidateClass = candidate.loadClass();
						// 创建被导入类对象，如果该类实现了 Aware BeanFactoryAware EnvironmentAware ResourceLoaderAware 接口会在这里回调
						...
						// 会推迟执行，并不会现在解析，而是等到当前配置类所属批次都解析完了之后再执行
						...
						ImportBeanDefinitionRegistrar registrar =
								ParserStrategyUtils.instantiateClass(candidateClass, ImportBeanDefinitionRegistrar.class,
										this.environment, this.resourceLoader, this.registry);
						// 先暂存起来，等到所有的配置类都解析完成了，再来调用解析这些配置类
						configClass.addImportBeanDefinitionRegistrar(registrar, currentSourceClass.getMetadata());
					}
					else {
						this.importStack.registerImport(currentSourceClass.getMetadata(), candidate.getMetadata().getClassName());
						// 如果当前导入的配置类不是一个 ImportSelector 类型的，就按照普通配置类去解析
